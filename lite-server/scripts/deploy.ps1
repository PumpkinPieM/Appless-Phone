...
if ($ConfigPath.Length -eq 0) {
  $ConfigPath = Join-Path $projectRoot 'examples\mock\config.json'
}
...
$server = Join-Path $BuildDir 'lite-server'
$tests = Join-Path $BuildDir 'lite-server-tests'
$vendor = Join-Path $BuildDir 'liblite_llm.so'
...
foreach ($required in @($server, $vendor, $libcxx, $ConfigPath)) {
...
foreach ($file in @($server, $vendor, $libcxx)) {
