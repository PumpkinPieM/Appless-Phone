...
void PrintUsage(const char *program)
{
    std::cout
        ...
        << "\nOptions:\n"
        << "  --config PATH                  Vendor config passed to CreateFromConfig\n"
...
int main(int argc, char **argv)
{
    ...
    std::unique_ptr<appless::lite_server::Model> model = appless::lite_server::CreateModel();
    if (!model->Build(arguments.config_path, &error)) {
        std::cerr << "lite-server: unable to build model: " << error << '\n';
        return 3;
    }
    std::cerr << "lite-server: model built from " << arguments.config_path << '\n';

    std::signal(SIGINT, HandleSignal);
    std::signal(SIGTERM, HandleSignal);

    appless::lite_server::ReverseClient client(arguments.connection, *model, ShouldStop);
    return client.Run();
}
