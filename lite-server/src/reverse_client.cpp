...
ReverseClient::ReverseClient(ReverseClientOptions options, Model &model, std::function<bool()> should_stop)
    : options_(std::move(options)), model_(model), should_stop_(std::move(should_stop))
{
}
...
bool ReverseClient::RunSession(int socket_fd, std::string *error)
{
    ...
    if (!SendFrame(socket_fd, protocol::BuildReadyStatus(), error)) {
        return true;
    }
    while (!should_stop_()) {
        std::string payload;
        ...
        if (!HandleMessage(socket_fd, payload, error)) {
            return true;
        }
    }
...
bool ReverseClient::HandleMessage(int socket_fd, const std::string &payload, std::string *error)
{
    JsonValue message;
    if (!ParseJson(payload, &message, error) || message.type() != JsonValue::Type::Object) {
    ...
    if (type->string() == "ping") {
        return SendFrame(socket_fd, protocol::BuildPong(), error);
    }
    ...
    if (body == nullptr || body->type() != JsonValue::Type::String) {
        return SendFrame(socket_fd, protocol::BuildError(id->string(), 400, "chat request has no body"), error);
    }

    ChatRequest chat_request;
    std::string request_error;
    if (!ParseChatRequest(body->string(), &chat_request, &request_error)) {
        return SendFrame(socket_fd, protocol::BuildError(id->string(), 400, request_error), error);
    }
    const GenerationResult generation = model_.Generate(chat_request.prompt);
    if (!generation.ok) {
        return SendFrame(socket_fd, protocol::BuildError(id->string(), 500, generation.error), error);
    }
    const std::string response = BuildChatCompletion(id->string(), chat_request.model, generation.text);
    std::cerr << "lite-server: completed request id=" << id->string()
              << " promptBytes=" << chat_request.prompt.size()
              << " responseBytes=" << generation.text.size() << '\n';
    return SendFrame(socket_fd, protocol::BuildChatResponse(id->string(), 200,
        "application/json; charset=utf-8", response), error);
}
