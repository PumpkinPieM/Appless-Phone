...
std::string LiteLlm::Generate(const std::string &prompt)
{
    ...
    const std::string requestBody =
        "{\"model\":\"" + EscapeJson(model_) + "\",\"messages\":[{\"role\":\"user\",\"content\":\"" +
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, MOCK_LOG_DOMAIN, MOCK_LOG_TAG,
        "DeepSeek Generate completed responseBytes=%{public}zu", content.size());
    return content;
}
} // namespace lite_llm
